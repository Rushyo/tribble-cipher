...
* No integrity, meaning that an attacker could alter the ciphertext in transit. This is generally A Bad Thing(tm), but not an issue for every use case. You can add a MAC yourself (a hash of the ciphertext + key) to validate the ciphertext, and I may consider writing this mode in to the libraries in future.
